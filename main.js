...
let wordsList = function (str, subStr) {
  ...
  let strNew = str.replace(/\.|,|\?|!|:|;|"/gi, '').toLowerCase().split(' ').filter((arrItem) => arrItem.indexOf(subStr) > -1);

  let result = new Set();
  strNew.forEach(function (i){
    result.add(i);
  });
  return result;
};
// console.log(wordsList(str, 'lore')); // {"lorem", "dolores", "doloremque", "dolore", "dolorem"}
// console.log(wordsList(str, 'no')); // {"non", "nostrum", "nobis"}
// console.log(wordsList(str, 'rep')); // {"repellendus", "repudiandae", "repellat", "reprehenderit"}
...
let getLocalDate = (date, isSeconds = false, isISO = false) => {
  ...
  if (!isISO) result = isSeconds ? date.toLocaleString() : date.toLocaleString().replace(regexp, '');
  else {
    ...
    result = isSeconds ? `${year}-${month}-${day}, ${hour}:${minutes}:${seconds}`
...
let Car = function (engine, model, name, year) {
  ...
  let car = new Car(1400, 'Fabia', 'Skoda', 2008);
  let car2 = new Car(5000, 'X5', 'BMW', 2020);
...
testPerformance = function (iterations, func) {
    let date = Date.now();
  ...
  function test1() {
   let strTest = str;
    while (strTest.indexOf('o') !== -1) strTest = strTest.replace('o', '');
    while (strTest.indexOf('a') !== -1) strTest = strTest.replace('a', '');
    while (strTest.indexOf('e') !== -1) strTest = strTest.replace('e', '');
    while (strTest.indexOf('u') !== -1) strTest = strTest.replace('u', '');
    while (strTest.indexOf('i') !== -1) strTest = strTest.replace('i', '');
    }
      // данная функция необходима для корректного тестирования кода
    function test2() {
